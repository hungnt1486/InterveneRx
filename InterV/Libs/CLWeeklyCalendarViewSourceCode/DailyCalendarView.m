...
-(void)markSelected:(BOOL)blnSelected
{
    ...
    if([self.date isDateToday]){
        self.dateLabelContainer.backgroundColor = (blnSelected)?[UIColor whiteColor]: [UIColor colorWithHex:0x12465C];
        
        self.dateLabel.textColor = (blnSelected)?[UIColor colorWithHex:0x12465C]:[UIColor whiteColor];
    }else{
...
//        self.dateLabel.textColor = (blnSelected)?[UIColor colorWithRed:52.0/255.0 green:161.0/255.0 blue:255.0/255.0 alpha:1.0]:[self colorByDate];
        self.dateLabel.textColor = [Settings setColorText];
    }
...
-(UIColor *)colorByDate
{
    return [self.date isPastDate]?[UIColor colorWithHex:0x12465C]:[UIColor whiteColor];
}
